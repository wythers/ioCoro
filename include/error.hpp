...
/**
 * Although forbid the signal PIPE, we must leave an error to reflect
 * what happened
 */
...
public:
  ...
  char const* name() const noexcept override { return "socket_closed_error"; }

  virtual ~SDstate() {}
};

inline static SDstate const socket_closed_error_impl{};

inline std::error_category const& get_socket_closed_error_category()
{
  return socket_closed_error_impl;
}
